...
def performance_metrics(imputed_values, true_values):
    ...
    # If Numerical Feature
        ...
        RMSE = sqrt(mean_squared_error(true_values, imputed_values)/np.var(true_values))
...
**NRMSE** : *results sorted from best to worst*

``` {r echo = TRUE, message = FALSE}

subset <- final_results[final_results$performance_type == 'nrmse', ]
head(subset[order(subset$performance, decreasing = FALSE), c("performance", "performance_type", "model", "type", "% drop")], 15)

```

**Accuracy** : *results sorted from best to worst*

``` {r echo = TRUE, message = FALSE}

subset <- final_results[final_results$performance_type == 'accuracy', ]
head(subset[order(subset$performance, decreasing = TRUE), c("performance", "performance_type", "model", "type", "% drop")], 15)
...
``` {r echo = FALSE}
  ...
  # a) Include additional column(s) for prediction
  # columns_predictions_if_cat <- cat_vec[!cat_vec %in% columns]
  # columns_predictions_if_cont <- cont_vec[!cont_vec %in% columns]
  # columns_predictions_if_both <- c(columns_predictions_if_cat, columns_predictions_if_cont)

  # b) Select appropriate case
  if (startsWith(columns[1], "cat") & startsWith(columns[2], "cat")) {
    pred_columns <- unique(c(columns, columns_predictions_if_cat))
  } else if (startsWith(columns[1], "cont") & startsWith(columns[2], "cont")) {
    pred_columns <- unique(c(columns, columns_predictions_if_cont))
