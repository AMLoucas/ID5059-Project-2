...
``` {python messages = FALSE, eval = FALSE, echo = FALSE}

true_df = pd.read_csv("data/train.csv")
nans_df = remove_random_values(true_df.copy(), ['cat0', 'cat1'], 0.2)
imp_df = univariate_imputation_method(nans_df[['cat0', 'cat1']], type = 'cat')
res_col1, res_col2 = performance_results(true_df, imp_df, nans_df, ['cat0', 'cat1'])
...
# note: all character types must be changed to factor
true_df <- as.data.frame(unclass(true_df))
nans_df <- as.data.frame(unclass(nans_df))
# str(nans_df)
...
columns_incl_predictors = c('cat0', 'cat1', 'cont0')
imp_df <- mice(nans_df[columns_incl_predictors], m = 5, maxit = 5)
...
imp_df <- missForest(nans_df[columns_incl_predictors])$ximp
mixError(imp_df, nans_df[columns_incl_predictors], true_df[columns_incl_predictors])
...
``` {python messages = FALSE, eval = FALSE}
...
#  imp_df: output directly from methods `mice()` or `missForest()`
#  columns: vector input containing strings representing column names which contain imputed values
#  method: string input 'mice' or 'missforest'
## OUTPUT
#  list containing:
#    if categorical - measures of accuracy and the confusion matrix
#    if numerical - measures of raw difference and percentage difference between
#                   imputed value and true value
performance_results_r <- function(true_df, imp_df, columns, method) {
  
  if (method == 'mice') {
    
    # set up
    m <- ncol(imp_df$imp[[columns[1]]])
    ...
    for (column in columns) {
      ...
      for (i in 1:m) {
        ...
        # - - - 
        # TODO: add an if statement here for the continuous case 
  ...
  return(list("Accuracy Scores" = accuracy,
...
##### Function for running the whole code 
