...
``` {python messages = FALSE, eval = TRUE, echo = FALSE}
# Correct data frame with true values
...
library(dplyr) # importin to use some functions to help wrangle data in appropriate structure.
# - - - - 
# 1. extract data
true_df <- py$true_df
nans_df <- py$nans_df
...
# note: all numeric that will be imputed must be unlisted.
nans_df <- nans_df %>%
  mutate(cont0 = unlist(cont0),
         cont1 = unlist(cont1),
         cont2 = unlist(cont2),
         cont3 = unlist(cont3),
         cont4 = unlist(cont4),
...
columns_incl_predictors = c('cont0', 'cat0', 'cat1')
# Placing a seed in the mice function for reproducible results. [For now]
imp_df <- mice(nans_df[columns_incl_predictors], m = 3, maxit = 3, seed=5059)
```
...
imp_df_forest <- missForest(nans_df[columns_incl_predictors])$ximp
...
pp <- performance_results_mice(true_df, imp_df, c('cont0', 'cat0'))
